...
export function MenuBar({
  ...
  const toggleActiveLayer = (patch: { visible?: boolean; locked?: boolean }) => {
  ...
  const menus: MenuDefinition[] = [
      {
        label: 'File',
        items: [
          { type: 'item', label: 'New...', action: () => setNewOpen(true), shortcut: 'Ctrl+N' },
          { type: 'item', label: 'Open Image...', action: () => openRef.current?.click(), shortcut: 'Ctrl+O' },
          { type: 'separator' },
          {
            type: 'item',
            label: 'Save as PNG',
            action: () =>
              exportFlattenedPng(state.layers, state.canvasWidth, state.canvasHeight),
          },
          { type: 'item', label: 'Save as JPEG...', action: () => setJpegOpen(true) },
          { type: 'separator' },
          {
            type: 'item',
      ...
      },
      {
        label: 'Edit',
        items: [
          { type: 'item', label: 'Undo', action: () => dispatch({ type: 'UNDO' }), shortcut: 'Ctrl+Z' },
          { type: 'item', label: 'Redo', action: () => dispatch({ type: 'REDO' }), shortcut: 'Ctrl+Y' },
          { type: 'separator' },
          {
            type: 'item',
            label: 'Cut',
          ...
          { type: 'item', label: 'Copy', action: onCopy, shortcut: 'Ctrl+C', disabled: !hasSelection },
          {
            type: 'item',
            label: 'Paste',
            action: onPaste,
          ...
          { type: 'separator' },
          { type: 'item', label: 'Select All', action: onSelectAll, shortcut: 'Ctrl+A' },
          { type: 'item', label: 'Deselect', action: onDeselect, shortcut: 'Ctrl+D' },
          {
            type: 'item',
      ...
      },
      {
        ...
        items: [
          ...
          { type: 'item', label: 'Sharpen', action: () => applyFilter(sharpen, 'Sharpen') },
          {
            type: 'item',
            label: 'Add Noise...',
            action: () => {
          ...
          { type: 'item', label: 'Emboss', action: () => applyFilter(emboss, 'Emboss') },
          { type: 'separator' },
          { type: 'item', label: 'Grayscale', action: () => applyFilter(grayscale, 'Grayscale') },
          { type: 'item', label: 'Invert Colors', action: () => applyFilter(invertColors, 'Invert') },
        ],
      },
      {
        label: 'Tools',
        items: [
          { type: 'item', label: 'Move Tool', action: () => setTool('move'), shortcut: 'V' },
          { type: 'item', label: 'Hand (Pan)', action: () => setTool('hand'), shortcut: 'H' },
          { type: 'item', label: 'Rectangular Marquee', action: () => setTool('marquee-rect'), shortcut: 'M' },
          { type: 'item', label: 'Elliptical Marquee', action: () => setTool('marquee-ellipse'), shortcut: 'O' },
          { type: 'item', label: 'Lasso', action: () => setTool('lasso'), shortcut: 'L' },
          { type: 'item', label: 'Polygon Lasso', action: () => setTool('polygon-lasso'), shortcut: '⇧L' },
          { type: 'item', label: 'Magic Wand', action: () => setTool('magic-wand'), shortcut: 'W' },
          { type: 'item', label: 'Crop', action: () => setTool('crop'), shortcut: 'C' },
          { type: 'separator' },
          { type: 'item', label: 'Brush', action: () => setTool('brush'), shortcut: 'B' },
          { type: 'item', label: 'Pencil', action: () => setTool('pencil'), shortcut: 'P' },
          { type: 'item', label: 'Eraser', action: () => setTool('eraser'), shortcut: 'E' },
          { type: 'item', label: 'Paint Bucket', action: () => setTool('fill'), shortcut: 'G' },
          { type: 'item', label: 'Gradient', action: () => setTool('gradient'), shortcut: '⇧G' },
          { type: 'item', label: 'Eyedropper', action: () => setTool('eyedropper'), shortcut: 'I' },
          { type: 'separator' },
          { type: 'item', label: 'Text', action: () => setTool('text'), shortcut: 'T' },
          { type: 'item', label: 'Rectangle', action: () => setTool('shape-rect'), shortcut: 'U' },
          { type: 'item', label: 'Ellipse', action: () => setTool('shape-ellipse'), shortcut: '⇧U' },
          { type: 'item', label: 'Line', action: () => setTool('shape-line'), shortcut: 'N' },
          { type: 'item', label: 'Arrow', action: () => setTool('shape-arrow'), shortcut: '⇧N' },
          { type: 'item', label: 'Zoom', action: () => setTool('zoom'), shortcut: 'Z' },
        ],
      },
      {
        label: 'View',
        items: [
          ...
          { type: 'item', label: 'Zoom 50%', action: () => dispatch({ type: 'SET_ZOOM', zoom: 50 }) },
          { type: 'item', label: 'Zoom 100%', action: () => dispatch({ type: 'SET_ZOOM', zoom: 100 }) },
          { type: 'item', label: 'Zoom 200%', action: () => dispatch({ type: 'SET_ZOOM', zoom: 200 }) },
          { type: 'separator' },
          {
            type: 'item',
    ...
    <>
      ...
      <Dialog open={newOpen} onOpenChange={setNewOpen}>
        <DialogContent>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <Label>Height</Label>
              <Input type="number" value={newH} onChange={(e) => setNewH(+e.target.value)} />
            </div>
      ...
      <Dialog open={jpegOpen} onOpenChange={setJpegOpen}>
        <DialogContent>
          ...
          <Button
            onClick={() => {
              exportJpeg(state.layers, state.canvasWidth, state.canvasHeight, jpegQ)
              setJpegOpen(false)
            }}
          >
            Export
      ...
      <Dialog open={canvasSizeOpen} onOpenChange={setCanvasSizeOpen}>
        <DialogContent>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label>Width</Label>
              <Input type="number" value={cw} onChange={(e) => setCw(+e.target.value)} />
            </div>
            <div>
              <Label>Height</Label>
              <Input type="number" value={ch} onChange={(e) => setCh(+e.target.value)} />
            </div>
          </div>
          <Button
            className="mt-4"
      ...
      <Dialog open={!!filterOpen} onOpenChange={() => setFilterOpen(null)}>
        <DialogContent>
          ...
          <Button
            onClick={() => {
              if (filterOpen === 'blur')
                applyFilter((ctx) => gaussianBlur(ctx, filterVal), 'Gaussian Blur')
