...
export async function loadEditorDraft(): Promise<EditorDraftCache | null> {
  ...
  const draft = await withStore<EditorDraftCache | undefined>('readonly', (store) =>
    store.get(DRAFT_KEY),
  )
...
export function snapshotToCachedLayer(snap: LayerSnapshot): CachedLayerSnapshot {
...
function loadImage(src: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => reject(new Error('Failed to decode cached layer image'))
    img.src = src
  })
}

export async function cachedLayerToLayer(snap: CachedLayerSnapshot): Promise<Layer> {
...
export async function layersFromDraft(draft: EditorDraftCache): Promise<Layer[]> {
